...
class InputHandle:
    ...
    def get_batch(self):
        if self.no_batch_left():
            ...
            return None
        image_height = int(self.image_width)
        input_batch = np.zeros(
            (self.minibatch_size, self.current_input_length, image_height, self.image_width, 3)).astype(
...
class DataProcess:
    ...
    def load_data(self, paths, mode='train'):
        ...
        for c_dir in c_dir_list: # handwaving
            ...
            for p_c_dir in p_c_dir_list: 
                ...
                for file in filelist: 
                    ...
                    frame_im = Image.open(os.path.join(dir_path, file)).convert("RGB")
        ...
        image_height = int(self.image_width)
        data = np.zeros((frames_np.shape[0], image_height, self.image_width , 3))
        for i in range(len(frames_np)):
            temp = np.float32(frames_np[i, :, :])
            data[i,:,:,:]=cv2.resize(temp,(self.image_width, image_height))/255
